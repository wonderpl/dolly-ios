...
- (IBAction) confirmTapped: (id) sender
{
    ...
    {
        case kLoginScreenStateLogin:
            valid = [self loginFormIsValidForUsername: self.userNameInputField
                                             password: self.passwordInputField];
            break;
            
        case kLoginScreenStateRegisterStepTwo:
            valid = [self registrationFormIsValidForEmail: self.registeringUserEmailInputField
                                                 userName: self.registeringUserNameInputField
                                                 password: self.registeringUserPasswordInputField
                                                       dd: self.ddInputField
                                                       mm: self.mmInputField
                                                     yyyy: self.yyyyInputField];
            break;
    ...
    switch (self.state)
    {
        case kLoginScreenStateLogin:
        {
            ...
            [self loginForUsername: self.userNameInputField.text
        ...
        case kLoginScreenStateRegisterStepTwo:
        {
            ...
            [self registerUserWithData: userData
                     completionHandler: ^(NSDictionary *dictionary) {
                         ...
                         if (self.avatarImage)
                         {
                             [self uploadAvatarImage: self.avatarImage
    ...
    [self doRequestUsernameAvailabilityForUsername: self.registeringUserNameInputField.text
                                 completionHandler: ^(NSDictionary *result) {
                                     ...
                                     if (availabilitynumber)
                                     {
                                         ...
                                         {
                                             [self.validUsernames
                                              addObject: self.registeringUserNameInputField.text];
                                             [self showRegistrationStep2];
                                         }
                                         else
                                         {
...
- (IBAction) textfieldDidChange: (id) sender
{
    ...
    if (sender == self.ddInputField && [self.ddInputField.text length] == 2)
    {
        ...
        {
            [self dateValidForDd: self.ddInputField
                              mm: self.mmInputField
                            yyyy: self.yyyyInputField];
        }
    }
